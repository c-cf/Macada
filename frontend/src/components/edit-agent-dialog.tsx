...
function agentToConfig(agent: Agent): Record<string, unknown> {
  return {
    name: agent.name,
...
interface EditAgentDialogProps {
  ...
  readonly onUpdated: (updated: Agent) => void;
}
...
export function EditAgentDialog({
  ...
  const [configText, setConfigText] = useState(toYaml(config));
        ...
        <div className="flex items-center justify-between px-6 pt-6 pb-4 shrink-0">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">
              {agent.name} &middot; v{agent.version}
            </p>
          </div>
          <button
            onClick={handleClose}
